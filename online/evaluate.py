...
# PATCHED: In2 wiring. Both arms run from this package now, so the arm
# must come from the env flag, exactly as training and export did.
...
def extract_predictions_and_surfaces(tuner_obj, data_loader, seed=1234):
    ...
    with torch.no_grad():
        for market_iv_surface, market_price_surface, context in data_loader:
            ...
            # PATCHED: In2 takes the stacked 2-channel input, built the same
            # way training built it (standardised sentiment vs constant fill).
            if _USE_IN2:
                from in2_adapter import make_in2_batch
                _x = make_in2_batch(scaled_input, context, use_sentiment=_USE_SENT)
                _, predicted_params = tuner_obj.model(_x)
            else:
...
def _evaluate_one(model_name, date_range, MODEL_REGISTRY, data_cache):
    ...
    # PATCHED: lexical sort put epoch_99 AFTER epoch_700, and the *_final_*
    # checkpoints were ignored entirely. Choose by numeric epoch, final first.
    def _ep(f):
        m = re.search(r"epoch_(\d+)\.pth$", os.path.basename(f))
        return int(m.group(1)) if m else -1
    finals = glob.glob(os.path.join(model_dir, "best_model_final_epoch_*.pth"))
    periodic = glob.glob(os.path.join(model_dir, "best_model_epoch_*.pth"))
    if finals:
        checkpoint_path = max(finals, key=_ep)
    elif periodic:
        checkpoint_path = max(periodic, key=_ep)
    elif os.path.exists(os.path.join(model_dir, "best_model.pth")):
    ...
    if _USE_IN2:
        ...
        plot_model = build_model_from_config_in2(selected_config, device, strict=False)
    else:
    ...
    if RUN_ZERO_SHOT_COMPARISON:
        if _USE_IN2:
            from in2_adapter import build_model_from_config_in2
            zero_model = build_model_from_config_in2(selected_config, device, strict=False)
        else:
...
if __name__ == "__main__":
    ...
    for model_name in CANDIDATE_MODELS:
        for date_range in CANDIDATE_DATE_RANGES:
            if os.environ.get('SDP_USE_IN2', '0') == '1':
